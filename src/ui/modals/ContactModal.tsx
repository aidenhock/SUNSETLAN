...
/**
 * The mailbox (Phase 4): a real contact form POSTing JSON to Formspree
 * with plain fetch — no SDK (no-new-deps rule; the endpoint is config,
 * never source). States run idle → sending → sent/failed; a hidden
 * honeypot (`_gotcha`, Formspree's own convention, so the server drops
 * it too) silently swallows bot submissions client-side. With no
 * endpoint configured the form renders disabled and says so — never a
 * silent failure. The mailto/social links stay as the fallback path.
 */
