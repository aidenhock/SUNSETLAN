...
- **Glitter lane (v3.13 — single-authority monotonic wedge, bounded height mapping; supersedes v3.12)**: ONE width authority — the corridor mask defines the lane's width everywhere: ~**0.5× disc width at the disc's base** (far end), widening smoothly and MONOTONICALLY toward the near shore (~2.5–3× disc width; both endpoints in `GLITTER` in planetConfig). **The entire specular footprint is clamped to the corridor** — every disc-light glint term multiplies by the mask, so grazing-angle spread near the horizon can never re-widen the lane (this kills the wide-narrow-wide hourglass; there is no disc glitter outside the corridor). **Height mapping, bounded**: low/setting = narrowest and most opaque (the vivid column); rising scales BOTH corridor endpoint widths up (~1.8× at inland-high) and eases opacity down to a FLOOR of ~45% — wider and more transparent, but never invisible while the disc shows. **The only kill is submergence**: lane × visible-disc fraction from the set-floor math, gone when the disc is gone. Moon parity, cool tint, night-gated. **Acceptance** (`setcheck.mjs`): far-end width 0.4–0.6× disc width touching the disc base; width samples at far/mid/near strictly increasing (no waist); a camera-orbit pair proving the lane stays anchored to the character; a high-vs-low pair (wide + faint ≥ the floor vs narrow + vivid); a submergence fade sequence. Both bodies.
