...
- **Sky gradients (v3.2)**: 3-stop vertical gradients (horizon / mid / zenith) per state, blended **stop-by-stop** by nightMix — never a 2-color blend. Day (permanent golden hour): horizon `#FF9E5E` → mid `#FFC98B` → zenith `#8FB8D8` (sunset over the sea from the day beach). Night: horizon `#24304F` → mid `#141B33` → zenith `#070A14` — blue into black, never pure black. Starting values — tune, then record the finals as palette tokens in the art direction. Fog + clear color always equal the **current horizon stop**.
...
- **Sun & moon at the waterline (v3.3)**: both bodies sit LOW planet-local (`CELESTIAL` in planetConfig — ~90° of arc from their own beach) so that from the sunset beach the sun hovers just above the sea horizon (setting into the ocean) and from the night beach the full moon does the same; each naturally sets into the sea behind you as you cross. From spawn the discs are below the horizon — the halo and gradient carry the mood there, and the intro swoop sees the discs from altitude. **Side gating**: sun disc + halo fade out by nightMix ~0.6; the moon is a faint pale ghost in residual daylight and reaches full brightness + its glow only past nightMix ~0.75 — it must never read as a bright second sun from the day half. The moon's halo stays cool blue-white and tighter than the sun's. **Stars**: seeded, night dome region, slight size variance (point-batch sizes), opacity fades in across nightMix 0.55 → 0.9; count by `qualityTier`.
...
**Head look-at (v3.3)**: when idle, the head pivot eases (~0.2 s exponential) toward the camera, clamped to ±60° yaw / ±25° pitch from body forward — beyond the clamp it eases back to neutral. When moving, the body faces travel (existing yaw) and the head follows travel with only a subtle ~10° glance toward the camera. Gentle idle head sway keeps standing still alive (eye blink skipped by design: the eyes are merged into the single head geometry node). Future NPCs get the same behavior through the shared rig.
