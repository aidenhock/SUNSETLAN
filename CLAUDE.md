...
- **Set floor + glitter visibility (v3.9)**: from ANY reachable spot the disc never exceeds ~45% submerged (waterline ≈ 40%, wading clamp tuned to ≤45% for both disc radii), and the glitter lane stays visible whenever the disc is — its distance fade extends past the horizon water so the lane connects the viewer to a half-set disc. Glitter reads clearly from the beach: wider lane, larger/softer glints (lower specular power, slightly more intensity), still a broken shimmer.
