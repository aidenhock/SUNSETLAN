...
- **Characters speak AC villager (Character 2.0, v3.15)**: ROUNDED, not blocky — flattened-sphere head, solid hair cap + fringe, egg torso, capsule limbs ending in **ball hands**, stubby capsule legs, lozenge shoes. Chibi ~2.2 heads tall; **the head carries the charm**: big vertical-oval eyes with a tiny white highlight each, micro-arc mouth, optional blush discs. **Shading exception (deliberate)**: characters render SMOOTH-shaded `MeshLambertMaterial` (`flatShading: false`) — the matte-plastic AC/Mii toy finish — while the world stays flat-faceted; smooth toys on stylized ground is Animal Crossing's own move. The rig is parameterized for BOTH villager body languages (hair styles incl. the feminine bob, tee+shorts or dress outfits) so NPCs reuse it. Still: primitives only, no glTF, no skeletons, pivot-group limbs (playbook §1 rounded recipe — binding), procedural animation (playbook §2), ≤ ~3k tris per character.
