...
- **Celestial arc (v3.7 — rise → high → set)**: disc elevation is a smooth function of the player's polar angle from island center — `CELESTIAL_ELEVATION_INLAND_DEG` (~45°) on the plateau, easing down across the beach band (terrainProfile band constants bound the ramp) to `CELESTIAL_ELEVATION_WATERLINE_DEG` (~12°) at the waterline — applied with ~0.6 s exponential smoothing so the sun sinks with you, never popping. Azimuths stay home (0 / 180); the solved disc latitude is CLAMPED to its home-side range so world rotation composes on top: walking 180 → 0 the moon sets into the sea behind you, the sun crests ahead, climbs high across the plateau, and eases down to hover over the water on the final approach. Both bodies share the rule. Side gating, maria, and stars unchanged.
