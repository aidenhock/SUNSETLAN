...
- **Glitter visibility (v3.11 — scales with sun height)**: the body is a DISC light, not a point — the specular falloff subtracts its angular radius (`angle = max(0, angleBetween(reflectDir, lightDir) − discRadius)`), so the lane can never render narrower than the disc and connects flush to its base at the horizon. Spread and intensity are driven by the body's apparent elevation above the ocean limb (already computed for the set floor, so the arc smoothing carries over): low/setting → tight bright lane (high power, full intensity); inland-high → broad soft shimmer field at ~40–50% intensity. Power/intensity endpoints live in `GLITTER` (planetConfig). Moon parity, cool tint, night-gated. The distance fade extends past the horizon water. **Acceptance**: `setcheck.mjs` asserts the lane's sampled width just below the limb ≥ the disc's rendered width AND lane pixels touch the disc's base — no gap — plus an inland-high broad-field capture, both sides.
