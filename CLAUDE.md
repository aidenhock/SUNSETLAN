...
- **Sky gradients (v3.4 — richer sunset, pixel-art-reference mood)**: 4-stop vertical ramps per state, blended **stop-by-stop** by nightMix. Day: deep orange horizon → pink → mauve → soft blue-violet zenith (tune, then record finals as palette tokens). Night: `#24304F` → `#1A2340` → `#10182E` → `#070A14` — blue into black, never pure black. Fog + clear color always equal the **current horizon stop**. Optional subtle banded quantization of the gradient (~10 steps, single flag) to echo the pixel-art references — keep only if it reads as intentional style.
- **Sun presentation (v3.4)**: brighter near-white core with a two-stage glow — tight warm inner + wide soft outer. Clouds near the sun's longitude carry a warm underlit tint (static per-cloud, by longitude); night-side clouds stay cool dark.
...
- **Sun & moon over the sea (v3.4)**: both bodies anchor by `CELESTIAL` in planetConfig with `CELESTIAL_ELEVATION_DEG` (~15°): from its home beach each disc's center sits ~12–18° above the sea horizon — in the sky, clearly over the water — and still sets into the sea behind you as you cross (a bit later than the waterline version). The disc latitude is solved numerically from the elevation at module init. **Side gating** unchanged: sun disc + glow fade out by nightMix ~0.6; the moon is a faint pale ghost in residual daylight, full past ~0.75, never a second sun; its halo stays cool blue-white and tighter than the sun's. **Moon detail (v3.4)**: 4–6 flat maria blotches in two darker grays (flat meshes on the disc — no image textures), subtle enough that the moon still glows. **Stars**: seeded, night dome region, slight size variance, opacity fades in across nightMix 0.55 → 0.9; count by `qualityTier`.
...
- **Glitter paths (v3.4)**: in the same water shader — a broken shimmer band on the sea leading from the shore toward the sun (warm, day side) and the moon (pale cool, night side), riding the live wave displacement so it sparkles and breaks up, fading with distance, gated by nightMix per body. No new meshes, no reflection systems.
