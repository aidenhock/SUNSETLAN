...
- **Proximity emitters + mixing** (per frame, ~0.5 s lerps): waves = shore-proximity bed crossfading successive bag picks into a seamless loop; gull cries emit FROM the orbiting gulls (bag-timed, day side); campfire crackle from the fire pool, night-scaled with the flicker. `musicGain = base × smoothstep(8, 20, arcTo(uke))` — inside 8 m the uke owns the soundscape; campfire ducks music to 0.6 (10 m → 4 m); media modals duck music to 0.2, restore on close. Everything respects mute.
