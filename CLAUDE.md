...
- **Glitter lane (v3.12 — a WEDGE anchored to the disc; supersedes the v3.11 elevation spread)**: the lane is a corridor mask over the wave shimmer — apex at the disc's base point on the ocean limb with angular width EQUAL to the disc's width there (never a point, never doubled), widening smoothly to ~`GLITTER.spread`× the disc width at the near shore. The wedge is filled by the existing disc-light specular shimmer, so it stays broken and follows player/camera, but the corridor guarantees ONE continuous connected lane from the disc's base to the near shore at every visible elevation. **Elevation only softens, never erases**: intensity eases from ~100% at set to ~70% inland-high (no broaden-into-faint-field). **The only fizzle-out is submergence**: the lane is multiplied by the visible-disc fraction from the set-floor math — full while ≥ ~50% visible, fading smoothly to zero as the disc sinks in the walk-away set, gone when the disc is gone. Moon parity, cool tint, night-gated. **Acceptance** (`setcheck.mjs`): waterline lane width just below the limb within ~1.0–1.3× disc width and touching the base; mid-island risen-body view shows one continuous lane viewer→horizon; a walk-away sequence fades the lane in lockstep with the disc, absent once submerged. Both bodies.
