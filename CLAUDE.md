...
- **Proximity emitters + mixing** (per frame, ~0.5 s lerps): waves = shore-proximity bed crossfading successive bag picks into a seamless loop; gull cries emit FROM the orbiting gulls (bag-timed, day side) with a REAL distance gradient — refDistance ~3, strong rolloff, and cry gain × smoothstep(30 m → 8 m to that gull), so flyovers swell and fade instead of playing flat; **campfire crackle is PURE proximity** — gain = smoothstep over the arc to the fire (12 m → 3 m), full volume up close regardless of nightMix (day/night has nothing to do with what a fire sounds like; the VISUAL flicker/emissive keep their night scaling). `musicGain = base × smoothstep(8, 20, arcTo(uke))` — inside 8 m the uke owns the soundscape; campfire ducks music to 0.6 (10 m → 4 m); media modals duck music to 0.2, restore on close. Everything respects mute.
